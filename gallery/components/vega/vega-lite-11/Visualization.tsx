...
import rawData from "@/components/vega/vega-lite-11/data.json";
...
export function Visualization() {
	const { questions, maxCount } = useMemo(() => {
		...
		const processedQuestions = medians.map((m: any) => {
			const questionValues = values.filter(
				(v: any) => v.name === m.name && typeof v.value === "number",
			);
			const ratingsMap = new Map<number, number>();
			for (let i = 1; i <= 5; i++) ratingsMap.set(i, 0);
			questionValues.forEach((v: any) => {
		...
		const mCount = Math.max(
			...processedQuestions.flatMap((q: any) => q.ratings.map((r: any) => r.count)),
		);
