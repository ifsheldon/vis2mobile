...
import {
	...
	type Species,
	scatterData,
} from "@/components/vega/vega-altair-19/penguinData";
				...
				<div className="flex-1 rounded-[32px] border border-white/50 bg-white/60 p-2 shadow-xl shadow-slate-200/50 backdrop-blur">
					<div className="relative h-full w-full">
						<svg
							...
							role="img"
							aria-labelledby="penguin-scatter-title"
						...
						>
							<title id="penguin-scatter-title">
								Penguin morphology scatterplot
							</title>
							<rect
								x={view.margin.left}
								y={view.margin.top}
							...
							{scatterData.map((point) => {
								...
								return (
									<motion.circle
										key={`${point.species}-${point.island}-${point.beakLength}-${point.beakDepth}-${point.flipperLength}-${point.bodyMass}-${point.sex ?? "NA"}`}
										cx={x}
										cy={y}
										initial={false}
										animate={{
