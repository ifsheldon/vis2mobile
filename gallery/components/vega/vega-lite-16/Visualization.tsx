...
import { drivingData } from "@/components/vega/vega-lite-16/data";
...
const CustomCard = ({ children, className }: CustomCardProps) => (
	<div
		className={cn(
			"bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden",
			className,
		)}
	>
		{children}
		...
		<div className="flex flex-col gap-4 w-full max-w-md mx-auto p-2 bg-zinc-50 min-h-[600px]">
			<div className="px-2 pt-4">
				<h2 className="text-xl font-bold text-zinc-900 tracking-tight">
					Driving vs. Gas Prices
				</h2>
				<p className="text-sm text-zinc-500 mt-1">
					The relationship between miles driven per capita and the cost of gas
					(1956–2010).
				</p>
				...
				<div className="flex justify-between items-center p-4">
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex flex-col items-end">
							...
							<span className="text-lg font-bold text-blue-900">
								{selectedPoint.miles.toLocaleString()}
							</span>
						</div>
					</div>
				</div>
			...
			<div className="w-full aspect-[4/5] relative px-0 bg-white rounded-xl border border-zinc-100 shadow-sm overflow-hidden">
				<ResponsiveContainer width="100%" height="100%">
					<ScatterChart margin={{ top: 40, right: 30, bottom: 50, left: 10 }}>
						<XAxis
							type="number"
							dataKey="miles"
							name="miles"
			...
			<div className="px-4 pb-8">
				<p className="text-xs text-zinc-400 italic leading-relaxed">
					Notice the "loops" in the trajectory. When gas prices rose sharply in
					the 1970s and 2000s, the steady increase in miles driven slowed or
					even reversed, showing how economic factors influence driving behavior
					over decades.
				</p>
			</div>
		</div>
