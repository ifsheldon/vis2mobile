            ...
            <div className="group flex flex-col gap-2 bg-white dark:bg-zinc-900 shadow-xl shadow-zinc-200/50 dark:shadow-black/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-2 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] w-12 hover:w-56 overflow-hidden">
                ...
                {NAV_ITEMS.map((item) => (
                    <a
                        key={item.id}
                        href={item.href}
                        className={`flex items-center gap-3 py-2 px-1.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors ${activeSection === item.id
                    ...
                    >
                        <span
                            className={`w-5 h-5 shrink-0 flex items-center justify-center text-[10px] font-bold border rounded transition-colors ${activeSection === item.id
