...
export function Visualization() {
  ...
  const [selectedMovie, setSelectedMovie] = useState<ProcessedData | null>(
    null,
  );
    ...
    const fetchData = async () => {
      try {
        ...
        const validData = json.filter(
          (d) => d["Major Genre"] && d["IMDB Rating"] != null,
        );
        const uniqueGenres = Array.from(
          new Set(validData.map((d) => d["Major Genre"])),
        ).sort();
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-zinc-950 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        {/* Sticky X-Axis Labels using absolute positioning to match chart margins */}
        <div className="h-6 bg-white dark:bg-zinc-900 border-b border-zinc-100 dark:border-zinc-800 z-20 relative">
          {[0, 2, 4, 6, 8, 10].map((tick) => (
            <span
              key={tick}
              className="absolute top-1/2 -translate-y-1/2 text-[9px] font-black text-zinc-400"
              style={{
                left: `calc(10px + (${tick} / 10) * (100% - 40px))`,
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none">
              {genres.map((genre, i) => (
                <div
                  key={genre}
                  className="absolute w-full flex items-start px-6 pt-3 border-t border-zinc-100 dark:border-zinc-800/50"
              ...
              >
                ...
                {genres.map((genre, i) => (
                  <ReferenceArea
                    ...
                    y2={i + 1}
                    fill={
                      i % 2 === 0 ? "transparent" : "rgba(148, 163, 184, 0.02)"
                    }
                    stroke="none"
                  />
                ))}
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-xl pointer-events-none">
        <p className="text-[10px] font-bold text-zinc-900 dark:text-white truncate max-w-[150px]">
          {data.title}
        </p>
