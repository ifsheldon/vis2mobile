...
export const rawData: CropYield[] = [
...
export const getAggregatedData = (): AggregatedData[] => {
  const map = new Map<string, AggregatedData>();
  for (const item of rawData) {
    let entry = map.get(item.variety);
    if (!entry) {
      entry = { variety: item.variety };
      map.set(item.variety, entry);
    }
    entry[item.site] = ((entry[item.site] as number) || 0) + item.yield;
  }
  return Array.from(map.values());
};

export const sites = Array.from(new Set(rawData.map((d) => d.site)));
